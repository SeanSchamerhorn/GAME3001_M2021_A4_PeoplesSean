...
bool CollisionManager::lineRectEdgeCheck(const glm::vec2 line_start, const glm::vec2 rect_start, const float rect_width, const float rect_height)
{
	...
	// return true if one line collides with the target's rectangle
	if (left || right || top || bottom)
	{
		state = true;
	}
...
bool CollisionManager::lineRectEdgeCheck(glm::vec2 line_start, const glm::vec2 rect_start, const float rect_width, const float rect_height, 
	glm::vec2 obs_start, float obs_width, float obs_height )
{
	/* Similar to the above method, this one creates four lines with each end point being a side of the target (assumed player). The obstacle data
	   is taken in as three new parameters. Then an SDL_IntersectRectAndLine check is done four times with each line and the obstacle rect. Only
	   returns true if all four checks pass meaning even one line that passes represents LOS on the target. */
	...
	const auto leftEdgeStart = glm::vec2(rx, ry);
	const auto leftEdgeEnd = glm::vec2(rx, ry + rh);
	auto leftEdgeMidPoint = Util::lerp(leftEdgeStart, leftEdgeEnd, 0.5f);
	...
	// line / rect comparisons
	const auto left = lineRectCheck(line_start, leftEdgeMidPoint, obs_start, obs_width, obs_height);
	const auto right = lineRectCheck(line_start, rightEdgeMidPoint, obs_start, obs_width, obs_height);
	const auto top = lineRectCheck(line_start, topEdgeMidPoint, obs_start, obs_width, obs_height);
	const auto bottom = lineRectCheck(line_start, bottomEdgeMidPoint, obs_start, obs_width, obs_height);

	// return true only if all lines collide with the obstacle's rectangle
	if (left && right && top && bottom)
	{
		state = true;
	}
...
{
	// This method iterates through all obstacles first and if all checks fail, then checks collision with ray and target.

	const auto start_point = agent->getTransform()->position;
	// Target.
	auto objectOffset = glm::vec2(target->getWidth() * 0.5f, target->getHeight() * 0.5f);
	const auto rect_start = target->getTransform()->position - objectOffset;
	const auto width = target->getWidth();
	const auto height = target->getHeight();

	// Check collision with objects. Stage 1.
	for (auto object : objects)
	{
		// Obstacle. Only for this case.
		auto obsOffset = glm::vec2(object->getWidth() * 0.5f, object->getHeight() * 0.5f);
		const auto obs_start = object->getTransform()->position - obsOffset;
		const auto obs_width = object->getWidth();
		const auto obs_height = object->getHeight();
		switch (agent->getType())
		{
		case AGENT:
			if (lineRectCheck(start_point, end_point, obs_start, obs_width, obs_height))
				return false;
			break;
		case PATH_NODE: // New check for obstacles and path nodes.
			if (lineRectCheck(start_point, end_point, obs_start, obs_width, obs_height))
				return false;
			/*if (lineRectEdgeCheck(start_point, rect_start, width, height, obs_start, obs_width, obs_height))
				return false;*/ 
			break;
		}
	}
	// Stage 2.
	// If we get here, there's no collision with obstacles, so check against target for an agent. But path nodes can automatically pass.
	switch (agent->getType())
	{
	case AGENT:
